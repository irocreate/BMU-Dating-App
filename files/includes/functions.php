...
if ( ! function_exists( 'plugin_admin_head_js' ) ) {
    function plugin_admin_head_js() {
        ...
        if ( is_admin() ) {
            ...
            wp_enqueue_script( 'jquery-datepicker', 'https://code.jquery.com/ui/1.11.2/jquery-ui.min.js' );
...
if ( ! function_exists( 'dsp_enqueue_script' ) ) {
    function dsp_enqueue_script() {
        ...
        wp_localize_script( 'parsley', 'validation_messages', array(
            'defaultMessage' => language_code( 'DSP_JS_VALIDATION_DEFALT_MESSAGE' ),
            'email'          => language_code( 'DSP_JS_VALIDATION_EMAIL' ),
            'url'            => language_code( 'DSP_JS_VALIDATION_URL' ),
            'number'         => language_code( 'DSP_JS_VALIDATION_VALID_NUMBER' ),
            'integer'        => language_code( 'DSP_JS_VALIDATION_VALID_INTEGER' ),
            'digits'         => language_code( 'DSP_JS_VALIDATION_VALID_DIGITS' ),
            'alphanum'       => language_code( 'DSP_JS_VALIDATION_VALID_ALPHANUM' ),
            'notblank'       => language_code( 'DSP_JS_VALIDATION_NOT_BLANK' ),
            'required'       => language_code( 'DSP_JS_VALIDATION_REQUIRED' ),
            'pattern'        => language_code( 'DSP_JS_VALIDATION_PATTERN' ),
            'min'            => language_code( 'DSP_JS_VALIDATION_MIN' ),
            'max'            => language_code( 'DSP_JS_VALIDATION_MAX' ),
            'range'          => language_code( 'DSP_JS_VALIDATION_RANGE' ),
            'minlength'      => language_code( 'DSP_JS_VALIDATION_MINLENGTH' ),
            'maxlength'      => language_code( 'DSP_JS_VALIDATION_MAXLENGTH' ),
            'length'         => language_code( 'DSP_JS_VALIDATION_LENGTH' ),
            'mincheck'       => language_code( 'DSP_JS_VALIDATION_MINCHECK' ),
            'maxcheck'       => language_code( 'DSP_JS_VALIDATION_MAXCHECK' ),
            'check'          => language_code( 'DSP_JS_VALIDATION_CHECK' ),
            'equalto'        => language_code( 'DSP_JS_VALIDATION_EQUAL_TO' )
        ) );
...
if ( ! function_exists( 'dsp_pagination' ) ) {
    function dsp_pagination( $total_result, $limit, $page, $adjacents, $page_name ) {
        ...
        if ( $lastpage > 1 ) {
            ...
            //previous button
            if ( $page > 1 ) {
                $pagination .= "<div><a class='disabled' href=\"" . $page_name . "page/$prev\">" . language_code( 'DSP_PREVIOUS' ) . "</a></div>";
            } else {
            ...
            //next button
            if ( $page < $counter - 1 ) {
                $pagination .= "<div class='disabled'><a href=\"" . $page_name . "page/$next\">" . language_code( 'DSP_NEXT' ) . "</a></div>";
            } else {
...
if ( ! function_exists( 'dsp_get_near_users' ) ) {
    function dsp_get_near_users( $filters = array() ) {
        ...
        $query = "SELECT profile.`user_id`,profile.`country_id`,profile.`city_id`,country.`name` country ,city.`name`  city ,profile. `make_private`";
        ...
        <li class="dsp-form-group dspdp-form-group ">
            ...
            <span class="dsp-sm-6 dspdp-col-sm-6">
                        <select class="dsp-multiple-select dsp-form-control dspdp-form-control chosen chzn-done"
                                ...
                                multiple="true">
	                        <?php /* ?><option value=" "><?php echo language_code('DSP_SELECT_OPTION'); ?></option><?php */ ?>
                            <?php
...
add_action( 'dsp_textbox_field', 'dsp_textbox_field_func', 10, 3 );
if ( ! function_exists( 'dsp_textbox_field_func' ) ) {
    function dsp_textbox_field_func( $profile_questions, $existOptions, $lang_code ) {
        global $wpdb, $current_user;
        $dsp_question_options_table = $wpdb->prefix . DSP_PROFILE_QUESTION_OPTIONS_TABLE;
...
if ( ! function_exists( 'dsp_display_question_by_order_func' ) ) {
    function dsp_display_question_by_order_func( $updateExitOption ) {
        global $wpdb;
        $dsp_session_language_table = $wpdb->prefix . DSP_SESSION_LANGUAGE_TABLE;
        $dsp_language_detail_table  = $wpdb->prefix . DSP_LANGUAGE_DETAILS_TABLE;
        if ( isset( $_SESSION['default_lang'] ) ) {
        ...
        foreach ( $myrows as $profile_questions ) {
            ...
            switch ( $profile_ques_type_id ) {
                ...
                case 2:
                    do_action( 'dsp_textbox_field', $profile_questions, $updateExitOption, $lang_code );
                    break;
