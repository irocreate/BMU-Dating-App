...
$colors = array('4570A5', '71568D', '4094AC', 'F7EFC6', 'C6BE8C', 'CC6600', '990000',
    '520000', 'BFBFC1', '808080'); // colors of the slices.
$shadow_height = 30; // Height on shadown.
...
for ($i = 0; $i < count($label); $i++) {
    if ($data[$i] / array_sum($data) < 0.1)
        $number[$i] = ' ' . number_format(($data[$i] / array_sum($data)) * 100, 0, ',', '.') . '%';
    else
        $number[$i] = number_format(($data[$i] / array_sum($data)) * 100, 0, ',', '.') . '%';
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $slice[] = array($start, $end, $shadow_color[$value_counter % count($shadow_color)],
        $fill_color[$value_counter % count($fill_color)]);
    $start = $end;
    $value_counter++;
}
