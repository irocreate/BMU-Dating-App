...
function wptouch_core_header_enqueue() {
    ...
    if (!bnc_wptouch_is_exclusive()) {
        ...
        wp_enqueue_script('wptouch-core', '' . compat_get_plugin_url('dsp_dating') . '/mobile/core/core.js', array(
            'jquery'), '1.9.x');
        wp_head();
        ...
        echo "<script src='" . compat_get_plugin_url('dsp_dating') . "/mobile/core/core.js?wptouch' type='text/javascript' charset='utf-8'></script>\n";
    }
...
function wptouch_core_header_pages() {
    ...
    global $blog_id;
    foreach ($pages as $p) {
        if (file_exists(compat_get_plugin_dir('dsp_dating') . 'mobile/images/icon-pool/' . $p['icon'])) {
            $image = compat_get_plugin_url('dsp_dating') . '/images/icon-pool/' . $p['icon'];
        } else {
...
function wptouch_core_header_rss() {
    if (bnc_is_rss_enabled()) {
        echo sprintf(__("%sRSS Feed%s", "wptouch"), '<li><a href="' . get_bloginfo('rss2_url') . '"><img src="' . compat_get_plugin_url('dsp_dating') . '/images/icon-pool/RSS.png" alt="" />', '</a></li>');
    }
}

function wptouch_core_header_email() {
    if (bnc_is_email_enabled()) {
        echo sprintf(__("%sE-Mail%s", "wptouch"), '<li><a href="mailto:' . get_bloginfo('admin_email') . '"><img src="' . compat_get_plugin_url('dsp_dating') . '/images/icon-pool/Mail.png" alt="" />', '</a></li>');
    }
}
...
function wptouch_twitter_link() {
    echo '<li><a href="javascript:(function(){var%20f=false,t=true,a=f,b=f,u=\'\',w=window,d=document,g=w.open(),p,linkArr=d.getElementsByTagName(\'link\');for(var%20i=0;i%3ClinkArr.length&&!a;i++){var%20l=linkArr[i];for(var%20x=0;x%3Cl.attributes.length;x++){if(l.attributes[x].nodeName.toLowerCase()==\'rel\'){p=l.attributes[x].nodeValue.split(\'%20\');for(y=0;y%3Cp.length;y++){if(p[y]==\'short_url\'||p[y]==\'shorturl\'||p[y]==\'shortlink\'){a=t;}}}if(l.attributes[x].nodeName.toLowerCase()==\'rev\'&&l.attributes[x].nodeValue==\'canonical\'){a=t;}if(a){u=l.href;}}}if(a){go(u);}else{var%20h=d.getElementsByTagName(\'head\')[0]||d.documentElement,s=d.createElement(\'script\');s.src=\'https://api.bit.ly/shorten?callback=bxtShCb&longUrl=\'+encodeURIComponent(window.location.href)+\'&version=2.0.1&login=amoebe&apiKey=R_60a24cf53d0d1913c5708ea73fa69684\';s.charSet=\'utf-8\';h.appendChild(s);}bxtShCb=function(data){var%20rs,r;for(r%20in%20data.results){rs=data.results[r];break;}go(rs[\'shortUrl\']);};function%20go(u){return%20g.document.location.href=(\'https://mobile.twitter.com/home/?status=\'+encodeURIComponent(document.title+\'%20\'+u));}})();" id="otweet"></a></li>';
}

function wptouch_facebook_link() {
    echo "<li><a href=\"javascript:var%20d=document,f='https://www.facebook.com/share',l=d.location,e=encodeURIComponent,p='.php?src=bm&v=4&i=1297484757&u='+e(l.href)+'&t='+e(d.title);1;try{if%20(!/^(.*\.)?facebook\.[^.]*$/.test(l.host))throw(0);share_internal_bookmarklet(p)}catch(z)%20{a=function()%20{if%20(!window.open(f+'r'+p,'sharer','toolbar=0,status=0,resizable=1,width=626,height=436'))l.href=f+p};if%20(/Firefox/.test(navigator.userAgent))setTimeout(a,0);else{a()}}void(0)\" id=\"facebook\"></a></li>";
}
...
function wptouch_core_footer_switch_link() {
    ...
    global $wpdb;
    $DSP_POST = $wpdb->prefix . "posts";
    $post_page_title_ID = $wpdb->get_row("SELECT * FROM $DSP_POST WHERE post_name='members'");
    $root_link = get_bloginfo('url') . "/?page_id=" . $post_page_title_ID->ID;
    echo '<script type="text/javascript">function switch_delayer() { window.location = "' . $root_link . '/&view=normal&redirect_nonce=' . wp_create_nonce('redirect') . '&redirect=' . urlencode($_SERVER['REQUEST_URI']) . '"}</script>';
    echo ' <a  onclick="wptouch_switch_confirmation();" href="javascript:return false;" style="color:#FA7000">Desktop Version</a>';
}
