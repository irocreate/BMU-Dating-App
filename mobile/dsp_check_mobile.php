...
require_once( 'include_mobile/plugin.php' );
require_once( 'include_mobile/compat.php' );
//echo '<br>hi1';
...
$wptouch_defaults = array(
    ...
    'header-background-color' => 'FA7000',
...
//    classic-wptouch-bg for classic back ground
...
class WPtouchPlugin {
    ...
    function __construct() {
        $this->my = 'sweet';
        //echo '<br>i WPtouchPlugin called<br>';
    ...
    function wptouch_parse_request($wp) {
        if (array_key_exists("dsp_dating", $wp->query_vars)) {
            switch ($wp->query_vars["dsp_dating"]) {
                case "upload":
    ...
    function bnc_head() {
        ...
        if ($this->applemobile || $this->desired_view == 'normal') {
            ...
            echo "<link rel='stylesheet' type='text/css' href='" . compat_get_plugin_url('dsp_dating') . "/mobile/core/core-css/wptouch-switch-link.css'></link>\n";
        }
    }

    function bnc_do_redirect() {
        global $post;
        // $loc='mobile/profile_header.php';
        //	 //header('Location: ' . $loc);
        ...
        //echo '<br>bnc_do_redirect<br>';
        if ($this->applemobile || $this->desired_view == 'mobile') {
            //echo 'bnc_do_redirect_checkmobile<br>';
            ...
            if ($wpdb->get_var("show tables like '$DSP_GENERAL_SETTINGS_TABLE'") == $DSP_GENERAL_SETTINGS_TABLE) {
                // get the page _id of the member page
                $page_ID = $wpdb->get_var("SELECT setting_value FROM $DSP_GENERAL_SETTINGS_TABLE WHERE setting_name='member_page_id'");
                // if we don't have page id then we will not check if call page is equal to our member page id
                if ($page_ID) {
                    $call_page_id = $wp_query->post->ID; //fetch post query string id
                    // here we are checking the call pge id with the member page id 
                    //beacuse in mobile version we just need to show only member page not any other page
                    if ($call_page_id != $page_ID) {
    ...
    function bnc_check_switch_redirect() {
        //echo '<br>bnc_check_switch_redirect';
        if (isset($_GET['view']) && isset($_GET['redirect'])) {
            if (isset($_GET['redirect_nonce'])) {
                $nonce = $_GET['redirect_nonce'];
                if (!wp_verify_nonce($nonce, 'redirect')) {
                    _e('Nonce failure', 'wptouch');
                    die;
                }
                $protocol = ( $_SERVER['HTTPS'] == 'on' ) ? 'https://' : 'http://';
                $redirect_location = $protocol . $_SERVER['SERVER_NAME'] . $_GET['redirect'];
    ...
    function handle_footer_done($content) {
        if (function_exists('twentyeleven_setup')) {
            return str_replace("WordPress</a>", "WordPress</a> <a href='https://www.wordpress.org/extend/plugins/wptouch'>" . sprintf(__('and %s', 'wptouch'), "WPtouch") . "</a>", $content);
        } else if (function_exists('twentyten_setup')) {
    ...
    function detectAppleMobile($query = '') {
        ...
        $devfile = compat_get_plugin_dir('dsp_dating') . '/include/developer.mode';
        //echo $devfile;
        ...
        if (isset($_GET['view'])) {
            //echo '618 detectAppleMobile view set=>'.$_GET[ 'view' ];
            if ($_GET['view'] == 'mobile') {
                //echo '<br>view';
                //$this->applemobile = true; /// for mobile
                $this->desired_view = 'mobile';
            } else if ($_GET['view'] == 'normal') {
                $this->applemobile = false;
                //echo '<br>mb='.$this->applemobile;
            }
        }
        ...
        //echo '<br>634 detectAppleMobile view=>'.$this->desired_view;
    }

    function get_stylesheet($stylesheet) {
        //echo '638 get_stylesheet applemobile<br>=>'.$this->applemobile.'    view=> '.$this->desired_view;
        if ($this->applemobile || $this->desired_view == 'mobile') {
            return 'default';
        } else {
            return $stylesheet;
    ...
    function get_template_directory($value) {
        $theme_root = compat_get_plugin_dir('dsp_dating');
        if ($this->applemobile || $this->desired_view === 'mobile') {

            return $theme_root . '/mobile';
        } else {
            return $value;
        }
    }
    ...
    function theme_root_uri($url) {
        $dir = compat_get_plugin_url('dsp_dating') . "/mobile";
        //echo $dir;
        if ($this->applemobile || $this->desired_view === 'mobile') {
            return $dir;
        } else {
...
function wptouch_switch() {
    ...
    if (bnc_is_iphone() || $wptouch_plugin->desired_view == 'normal') {
        ...
        echo "<a onclick=\"document.getElementById('switch-on').style.display='block';document.getElementById('switch-off').style.display='none';\" href=\"" . get_bloginfo('url') . "/?view=mobile&redirect_nonce=" . wp_create_nonce('redirect') . "&redirect=" . urlencode($_SERVER['REQUEST_URI']) . "\"><img id=\"switch-on\" src=\"" . compat_get_plugin_url('wptouch') . "/mobile/core/core-images/on.jpg\" alt=\"on switch image\" class=\"wptouch-switch-image\" style=\"display:none\" /><img id=\"switch-off\" src=\"" . compat_get_plugin_url('wptouch') . "/mobile/core/core-images/off.jpg\" alt=\"off switch image\" class=\"wptouch-switch-image\" /></a>";
        echo '</div>';
    }
}
...
function bnc_get_title_image() {
    ...
    if (file_exists(compat_get_plugin_dir('dsp_dating') . '/images/icon-pool/' . $title_image)) {
        $image = compat_get_plugin_url('dsp_dating') . '/images/icon-pool/' . $title_image;
    } else if (file_exists(compat_get_upload_dir() . '/dsp_dating/custom-icons/' . $title_image)) {
        $image = compat_get_upload_url() . '/dsp_dating/custom-icons/' . $title_image;
    }
    return $image;
}
