...
$count_online_users = $wpdb->get_var("SELECT COUNT(*) FROM $DSP_USER_ONLINE_TABLE WHERE session='$session_id'");
...
if ($count_online_users == "0") {
    ...
    $wpdb->query("UPDATE $DSP_USER_ONLINE_TABLE SET time='$time',user_id='$user_id',status='$status' WHERE session = '$session_id'");
}
