...
if ($profile_pageurl == "send_wink_msg") {
    ...
    if ($check_free_mode->setting_status == "N") {  // free mode is off 
        if ($check_free_trail_mode->setting_status == "N") { // free trial mode is off 
            ...
            if ($check_membership_msg == "Expired") {
                ...
                include(WP_DSP_ABSPATH . "/m/dsp_print_message.php");
            } else if ($check_membership_msg == "Access") {

                include(WP_DSP_ABSPATH . "/m/send_wink_message.php");
    ...
    include_once(WP_DSP_ABSPATH . '/m/country_st_ct.php'); // to change the state and city according to country
    ...
    if ($check_couples_mode->setting_status == 'Y') {
        if (($gender == 'C') && $edit_profile_pageurl == 'partner_profile') {
            //include(WP_DSP_ABSPATH."m/edit_profile_header.php");
            include(WP_DSP_ABSPATH . "m/edit_partner_profile_setup.php");
        } else if (($gender == 'M') || ($gender == 'F')) {

            include(WP_DSP_ABSPATH . "m/edit_profile_setup.php");
        } else {
            //include(WP_DSP_ABSPATH."m/edit_profile.php");
            include(WP_DSP_ABSPATH . "m/edit_profile_setup.php");
        }
    } else {
        include(WP_DSP_ABSPATH . "m/edit_profile_setup.php");
    ...
    include(WP_DSP_ABSPATH . "m/edit_profile_general.php");
} else if ($profile_pageurl == 'edit_picture') {
    include(WP_DSP_ABSPATH . "m/edit_profile_picture.php");
} else if ($profile_pageurl == 'edit_profile_question') {
    include(WP_DSP_ABSPATH . "m/edit_profile_question.php");
        ...
        if ($check_limit_profile_mode->setting_status == 'Y') {
            if (($user_id != $member_id)) {
                ...
                if ($value <= $no_of_profile) {
                    ...
                    if ($exist_member > 0) {
                        include(WP_DSP_ABSPATH . "m/view_couples_profile_header.php");
                    } else {
                        if ($payment_status != 1) {
                            ?>		
    ...
    if ($view == "frnd_request") {
        include(WP_DSP_ABSPATH . "m/dsp_frnd_request.php");
    } else if ($view == "comments") {
        include(WP_DSP_ABSPATH . "m/dsp_view_comments.php");
    } else if ($view == "viewComments") {
        include(WP_DSP_ABSPATH . "m/showComment.php");
    } else if ($view == "virtual_gifts") {
        include(WP_DSP_ABSPATH . "m/dsp_view_virtual_gifts.php");
    } else if ($view == "viewGift") {
        include(WP_DSP_ABSPATH . "m/showGift.php");
    } else if ($view == "view_winks") {
        include(WP_DSP_ABSPATH . "m/dsp_view_winks.php");
    } else if ($view == "viewWink") {
        include(WP_DSP_ABSPATH . "m/showWink.php");
    } else {

        include(WP_DSP_ABSPATH . "m/dspAlert.php");
    }
    ...
    include(WP_DSP_ABSPATH . "m/dsp_notification.php");
