/**Be careful, code below could not be changed!!!**/
function getURLLink()
{
    var urlLink;
    try
...
function getParamatersOfURLLink(urlLink)
{
    var parameters = "";
...
function openSWF(urlValue, widthValue, heightValue, divId, objectId, flashVersion)
{
    if (divId == undefined)
    {
        divId = "webmessenger";
    ...
    {
        appSrc = urlValue.substring(0, indexOfInterrogation);
        parameter = urlValue.substring(indexOfInterrogation + 1);
    }
    else
    {
        appSrc = urlValue;
    ...
    document.write('</DIV>');
    var flashvars = {};
    var params = {FlashVars: parameter, allowScriptAccess: "always"};
    var attributes = {id: objectId, name: objectId, allowScriptAccess: "always"};
    swfobject.embedSWF(urlStr, divId, widthValue, heightValue, flashVersion, "expressInstall.swf", flashvars, params, attributes);
}
