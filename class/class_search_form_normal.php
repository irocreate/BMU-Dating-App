	...
	class wpdating_search_form {
	    ...
	    */
		...
		public function fetch_from_setting_database($settings_name) {
			...
			$query_return = $this->_db->get_var($this->_db->prepare($query, $settings_name));
			return $query_return;
		}
		...
		public function fetch_from_user_database($settings_name, $user_id) {
			...
			$query_return = $this->_db->get_var($this->_db->prepare($query, $user_id));
			return $query_return;
		}
  		...
  		public function dsp_get_new_country_field() {
            ...
            $content = '';
            $distance_feature = $this->fetch_from_setting_database('distance_feature');
            $placeholder = $distance_feature ? language_code('DSP_SEARCH_BY_PLACE_ZIPCODE_COUNTRY') : language_code('DSP_COUNTRY');
			...
			if (is_user_logged_in() ) { ?>
				...
				<div class="dsp-filter-container">
					...
					<div class="dsp-lg-2 dsp-md-2">
						<h4><?php echo language_code('DSP_QUICK_SEARCH'); ?></h4>
                    </div>
